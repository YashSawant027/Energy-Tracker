...
const ElectricityConsumptionCalculator = () => {
  ...
  const calculateResults = () => {
    const totalKWh = appliances.reduce((acc, appliance) => {
      return (
        acc +
        (parseFloat(appliance.kwh || 0) *
          parseFloat(appliance.hours || 0) *
          30)
      );
    }, 0);
    ...
    const savings =
      expectedBill > 0
        ? (((expectedBill - actualBill) / expectedBill) * 100).toFixed(2)
        : 0;
  ...
  const handleTrackAgain = () => {
    setEnergySource('');
    setEnergyProvider('');
    setAppliances([{ name: 'Light', kwh: '', hours: '' }, { name: 'Fan', kwh: '', hours: '' }]);
    setPeakUsage('');
    setHighSeason('');
    setMonthlyBill('');
    setResult({});
              ...
              <>
                {step === 1 && (
                  <>
                    ...
                    <button
                      onClick={() => setStep(2)}
                      disabled={!energySource || !energyProvider}
                      className={`px-4 py-2 rounded text-white ${
                        !energySource || !energyProvider
                    ...
                    {appliances.map((appliance, index) => (
                      <div key={index} className="grid grid-cols-3 gap-2 mb-2">
                        <input
                          ...
                          onChange={(e) =>
                            handleApplianceChange(index, 'name', e.target.value)
                          }
                          className="border p-2 rounded"
                        />
                        <input
                          type="number"
                          placeholder="e.g., 0.07 KWH"
                          value={appliance.kwh}
                          onChange={(e) =>
                            handleApplianceChange(index, 'kwh', e.target.value)
                          }
                          className="border p-2 rounded"
                        />
                        <input
                          ...
                          onChange={(e) =>
                            handleApplianceChange(index, 'hours', e.target.value)
                          }
                          className="border p-2 rounded"
                        />
                      </div>
                    ...
                    <div className="flex justify-between">
                      ...
                      <button
                        ...
                        disabled={appliances.some(
                          (a) => !a.name || !a.kwh || !a.hours
                        )}
                        className={`px-4 py-2 rounded text-white ${
                          appliances.some((a) => !a.name || !a.kwh || !a.hours)
                            ? 'bg-gray-400 cursor-not-allowed'
                            : 'bg-blue-500 hover:bg-blue-600'
                        }`}
                      >
                ...
                {step === 3 && (
                  <>
                    ...
                    <div className="flex justify-between">
                      ...
                      <button
                        onClick={() => setStep(4)}
                        disabled={!peakUsage || !highSeason}
                        className={`px-4 py-2 rounded text-white ${
                          !peakUsage || !highSeason
                            ? 'bg-gray-400 cursor-not-allowed'
                            : 'bg-blue-500 hover:bg-blue-600'
                        }`}
                      >
